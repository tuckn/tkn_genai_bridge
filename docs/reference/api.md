...
利用側の保存形式に変換する場合は、`data` と `record` を分け、生成内容の検証を済ませてから保存してください。
JSON本体だけを返す既存の関数に合わせる場合は `result.data` を返せますが、実行記録の保存は利用側で別途行います。
成功時・失敗時とも、利用量の `completeness`、`known_subtotal`、各項目の `null` と、コストの `unavailable_reason` を保持してください。
総量と小計の意味、旧記録の読み込み時の扱いは [利用量の完全性](costs.md#総量既知小計完全性) を参照してください。
保存済み記録の再利用や再生成は利用側で判断し、アプリ独自の生成条件も判定に含めます。
